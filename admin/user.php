<?php
    ...
    try {
        ...
        if ($search != NULL) {
            $items = query("SELECT * FROM users WHERE username LIKE :username", array(
                ':username' => $search
            ));
        }
        else {
            $items = query('SELECT * FROM users');
        }
                        ...
                        <div class="bg-secondary rounded h-100 p-4">
                            ...
                            <form method="post" action="newletter/sendemail.php">
                                <div class="table-responsive">
                                    <a href="user/create.php" class="btn btn-primary m-2">Create</a>
                                    <input type="submit" class="btn btn-danger m-2" value="Edit Selected Users Permissons">
                                    <form method="post" action="newletter/sendemail.php">        
                                        <input type="hidden" name="users" />
                                        <input type="submit" class="btn btn-danger m-2" value="Edit All User Permission">
                                    </form>
                                    <table class="table">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            <?php for ($i = 0, $size = count($users); $i < $size; $i++) { ?>
                                            <tr>
                                                <th scope="row"><input name="emails[]" type="checkbox" value="<?php echo $users[$i]['id']; ?>"></th>
                                                <td><?php echo $users[$i]['username']; ?></td>
                                                <td>
                                                    <a href="user/edit.php?id=<?php echo $users[$i]['id']; ?>" class="btn btn-primary m-2">Edit</a>    
                                                    <a href="user/delete.php?id=<?php echo $users[$i]['id']; ?>" class="btn btn-danger m-2">Delete</a>    
                                                </td>
                                            </tr>
                                            <?php } ?>
                                        </tbody>
