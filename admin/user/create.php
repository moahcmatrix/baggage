<?php
    ...
    try {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $user = query('SELECT * FROM users WHERE username=:username', array(
                'username' => $username
            ));
            ...
            if (!(hasErrors($eUsername) || hasErrors($ePassword) ||
                hasErrors($eConfirmPassword) || hasErrors($eUPermissions))) {        
                    query('INSERT INTO users(id, username, password) VALUES (NULL, :username, :password)', array(
                        ':username' => $username,
                        ':password' => $password
                    ));
                    $id = query('SELECT id FROM users WHERE username=:username AND password=:password', array(
                        ':username' => $username,
                        ':password' => $password
                    ));
