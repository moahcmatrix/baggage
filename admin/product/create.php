<?php
     ...
     try {
        $eName = $ePrice = $eDiscount = $eDescription = $eState = $eImage = [];
        $state = [];
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $exProduct = query('SELECT * FROM products WHERE name=:name', array(
                ":name" => $name
            ));
            ...
            if (!(hasErrors($eName) || hasErrors($ePrice) || hasErrors($eDiscount) ||
                hasErrors($eDescription) || hasErrors($eState)) || hasErrors($eImage)) {  
                      
                    query("INSERT INTO `products` (`id`, `name`, `image`, `price`, `discount`, `description`, `rate`, `active`) VALUES ('', :name, :image, :price, :discount, :description, '0', :active);", array(
                        ':name' => $name,
                        ':image' => $imageUrl,
                        ':price' => $price,
                        ':discount' => $discount,
